 ...
 */
boolean CarConfigFile::begin(char *configFileName)
{
  log_d("boolean CarConfigFile::begin(char *configFileName)");
  ...
  *_atEnd = true;
   ...
   */
  *_lineSize = 128;
  _line = (char *)ps_calloc(*_lineSize, sizeof(char));
  if (_line == 0)
  {
    log_e("out of memory");
    *_atEnd = true;
  ...
  if (!_file)
  {
    log_e("Could not open SD file: %s", configFileName);
    *_atEnd = true;
    return false;
  }
  ...
  *_atEnd = false;
  ...
  log_d("begin succeeded");

  return true;
}

/*
 * Cleans up our CarConfigFile object.
 */
void CarConfigFile::end()
{
  if (_file)
  {
    _file.close();
 ...
 */
boolean CarConfigFile::readNextSetting()
{
  int bint;
 ...
 */
boolean CarConfigFile::nameIs(const char *name)
{
 ...
 */
const char *CarConfigFile::getName()
{
 ...
 * WARNING: calling this when an error has occurred can crash your sketch.
 */
const char *CarConfigFile::getValue()
{
  if (*_lineLength <= 0 || *_valueIdx <= 1)
  {
    return 0;
...
char *CarConfigFile::copyValue()
{
  ...
  result = (char *) ps_malloc(length + 1);
  if (result == 0)
  {
 ...
 */
boolean CarConfigFile::getBooleanValue()
{
